...
The authoritative geocoding input uses `addressLine1`, `city`, `province`, and
Canada. No client-provided coordinates are accepted. Existing clients may continue
sending `postalCode` temporarily, but the backend ignores it.
...
`INVALID_DELIVERY_ADDRESS`. If the provider is unavailable, order creation continues
with `NEEDS_REVIEW` and null delivery coordinates.
...
municipality. Incomplete, partial, or low-confidence results remain invalid.
