...
const normalizePaymentMethodForFingerprint = (value: unknown): string => {
  const normalizedValue = normalizeString(value);

  return normalizedValue === LEGACY_ETRANSFER_PAYMENT_METHOD.toLowerCase()
    ? "etransfer"
    : normalizedValue;
};
...
export const preventDuplicatePublicOrderSubmission = async (
  ...
  if (reserved.length === 0) {
    res.status(409).json({
      ...
      message:
        "This order was already submitted. Please check your current order before trying again."
    });
    return;
  }
